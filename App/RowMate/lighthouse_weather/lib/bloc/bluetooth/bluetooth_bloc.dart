...
class BleBloc extends Bloc<BleEvent, BleState> {
  ...
  DeviceIdentifier _addressLighthouseStation =
      DeviceIdentifier('B8:27:EB:32:DE:88');
    ...
    _streamBleScan = _flutterBlue.scanResults.listen((results) {
      for (ScanResult result in results) {
        ...
        if (result.device.name == _nameLighthouseStation &&
            result.device.id == _addressLighthouseStation) {
  ...
  void setUID(String v) async {
    print('Setting up UID');
    _addressLighthouseStation = DeviceIdentifier(v);
  }
  ...
  BluetoothService getServiceByGuid(Guid guid) {
    return bleServices.firstWhere((service) => service.uuid == guid,
        orElse: null);
  }
